...
// Mock logger to avoid console spam during tests (but allow console for debugging)
jest.mock('../services/logger', () => ({
  info: jest.fn(),
  warn: jest.fn(),
  error: jest.fn((msg, data) => {
    // Still log errors to console for debugging test failures
    if (process.env.DEBUG_TESTS) {
      console.error('[Test Logger]', msg, data);
    }
  }),
  debug: jest.fn()
}));
